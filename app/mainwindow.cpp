    ...
    if (pluginInfo.isEmpty()) {
        ...
        if (type == "ui_qml") {
            ...
            if (!QFile::exists(qmlViewPath)) {
                ...
                // and a backend ctor may fire its first (token-gated)
                // capability_module.requestModule before ViewModuleHost emits ready().
                // Registering after ready (as we used to) races those first calls — they
                // reach capability_module's fail-closed gate before the token is known
                // and are rejected as unauthorized.
                LogosAPI* logosAPI = new LogosAPI("standalone", this);
