...
EOF
      ...
      # Built-in modules come from liblogos, which already ships them in the
      # layout logos_core expects (modules/<name>/manifest.json + library) —
      # see logos-liblogos nix/modules.nix and nix/bin.nix. main.cpp defaults
      # the modules dir to <bindir>/../modules, i.e. exactly $out/modules.
      mkdir -p $out/modules
      cp -rL "${logosLiblogos}"/modules/* $out/modules/
      chmod -R u+w $out/modules
      echo "Built-in modules: $(ls $out/modules | tr '\n' ' ')"
      if [ ! -d "$out/modules/capability_module" ]; then
        echo "Error: capability_module missing from $out/modules (expected from ${logosLiblogos}/modules)" >&2
        exit 1
      fi
